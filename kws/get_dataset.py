...
def min_max_norm(item):
  audio = item['audio']
  label = item['label']
  audio_norm = (audio - audio.min()) / (audio.max() - audio.min())
  return audio_norm, label
...
def get_training_data(Flags, get_waves=False, val_cal_subset=False):
  ...
  if get_waves:
    ...
    #ds_train = ds_train.map(min_max_norm)
    ds_train = ds_train.map(convert_dataset)
    #ds_test = ds_test.map(min_max_norm)
    ds_test = ds_test.map(convert_dataset)
    #ds_val = ds_val.map(min_max_norm)
