...
from utils.models import DSCnnSN
import math 
...
def main(args):
    ...
    if args.model == "PIT":
        ...
        if args.gumbel == "True":
            model = DSCnnSN(gumbel = True)
        else:
            model = DSCnnSN(gumbel = False)
        model = model.to(device)
    ...
    name = f"ck_kws_{args.model}"
    warmup_checkpoint = CheckPoint('./warmup_checkpoints', model, optimizer, 'max',fmt=name+'_{epoch:03d}.pt')
    skip_warmup = True
    if pathlib.Path(f'./warmup_checkpoints/final_best_warmup_kws_{args.model}.ckp').exists():
        warmup_checkpoint.load(f'./warmup_checkpoints/final_best_warmup_kws_{args.model}.ckp')
    ...
    if not skip_warmup:
        ...
        warmup_checkpoint.save(f'./warmup_checkpoints/final_best_warmup_kws_{args.model}.ckp')
    ...
    name = f"ck_kws_opt_{args.model}_{args.loss_type}_targets_{args.loss_elements}_{args.l}_size_{args.size_target}_lat_{args.latency_target}"
    ...
    name = f"best_final_ck_kws_opt_{args.model}_{args.loss_type}_targets_{args.loss_elements}_{args.l}_size_{args.size_target}_lat_{args.latency_target}.ckp"
