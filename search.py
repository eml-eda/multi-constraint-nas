...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        complexity_loss = torch.abs(sum(model.size_dict.values()) - args.size_target)
        complexity_loss *= args.cd
        # Do same for ops here
        loss += complexity_loss
        loss.backward()
        optimizer.step()
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tComplexity-Loss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss.item(), complexity_loss.item()))
            if args.dry_run:
                break
...
def main():
    ...
    parser.add_argument('--cd', type=float, default=0.0, metavar='CD',
                        help='complexity decay (default: 0.0)')
