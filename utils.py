...
def train_one_epoch(
        ...
        increment_cd_ops = None,
        arch_optimizer = None) -> Dict[str, float]:
    model.train()
    avgacc = AverageMeter('6.2f')
    avgloss = AverageMeter('2.5f')
    avglosstask = AverageMeter('2.5f')
...
# Definition of the function to train for one epoch
def train_one_epoch_finetuning(
        epoch: int,
        search: bool,
        model: nn.Module,
        criterion: nn.Module,
    ...
    with tqdm(total=len(train_dl), unit="batch") as tepoch:
        ...
        for audio, target in train_dl:
            ...
            loss_task = criterion(output, target)
            if search:
                loss_reg = cd_size * model.get_memory_layer_constraints()
                loss = loss_task + cd_size * model.get_memory_layer_constraints()
            else:
                loss = loss_task
                loss_reg = 0
            optimizer.zero_grad()
...
# Definition of the function to train for one epoch
def train_one_epoch_finetuning_sizeloss(
        epoch: int,
        search: bool,
        model: nn.Module,
        criterion: nn.Module,
